...
| 2 typed compiler/lineage | partial | semantic IR, resolver, type inference, contracts, generated tests, lineage/impact; nested `array`/`map`/`row` types parsed; `lineage --upstream/--downstream` filter direction; `impact` accepts model or column and renders consumers. Gaps: offline column lineage/impact need catalogue schemas; unsupported SQL is `Unknown` |
| 3 state-aware execution | partial | versions, skip/build/cached, stale plans. Source states are wired: `Adapter::source_state` reads Iceberg snapshots and CLI plan/apply/run enrich compilation with them; cached reuse covered by test. Gaps: snapshot state for non-Iceberg sources is unavailable; no cache *reuse* execution (classification only) |
| 4 incremental models | partial | `@incremental` strategies; append/merge; partition replaces touched partitions; time-window uses a committed watermark with typed predicate; schema classification forces full rebuilds; Trino `MERGE` verified live on Iceberg. Gaps: partition replace is column-list based (no partition-metadata pruning); overlap is stored but not applied; no schema/watermark test against every adapter |
| 5 Nessie + WAP | partial | Nessie client boundary (REST v2, live-verified), `promote`/`rollback`/promotion artifact. `apply --ref <candidate> --from <base>` auto-creates the branch and a branch-scoped Trino catalog and writes candidate data; tests and an optional diff gate are enforced at promotion; live Nessie + Trino/Iceberg E2E covers isolation, diff, promotion and stale rejection. Gaps: no schema-policy or Iceberg-snapshot audit gate, no candidate cleanup/rebase; Nessie Iceberg catalog has no views |
| 6 data diff | partial | keyed diff with per-column counts and configured policies/tolerances; real `TABLESAMPLE` sampling; partition-aware summary; populated schema diffs; stale-diff invalidation at promotion; live Trino coverage for keyed/tolerance/partition/sampled. Gaps: partition strategy compares partition row counts rather than metadata pruning; no example-value redaction (no examples emitted) |
