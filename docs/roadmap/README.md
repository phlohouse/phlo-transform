...
| dbt migration | done | `translate --from dbt` (phlo-transform-dbt): project/YAML/model/test/macro discovery, Jinja scan-classify (no execution), `ref`/`source`/`var`/`config` lowering, materialisation + incremental strategy mapping (merge/key, insert_overwrite/partition, append, watermark `is_incremental()` → `window`), generic-test conversion (`unique`/`not_null`/`accepted_values`/`relationships`/`unique_combination_of_columns`), `CLEAN`/`REVIEW`/`UNSUPPORTED` classification, human + `--json` report, `.phlo/migration/dbt-translation.json` manifest, `--out`/`--overwrite`/`--verify` (non-zero exit when the generated workspace fails `check`). See `docs/dbt-migration.md` and `docs/validation.md`. Gaps: `{% set %}`/`{% for %}`/custom macros are REVIEW (not evaluated); seeds, exposures, metrics, semantic models, snapshots unsupported; dbt `source()`/model `meta` beyond owner lands only in the manifest |
