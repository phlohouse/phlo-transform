...
| 1 MVP engine | partial | Trino adapter + plan/apply/run/test, scheduler, state, artifacts, cancellation, `--retries` backoff + `--resume`/`--retry-failed` continuations. Gaps: partition-level scheduler tests absent |
...
| 3 state-aware execution | partial | versions, skip/build/cached, stale plans. Source states are wired: `Adapter::source_state` reads Iceberg snapshots (schema-fingerprint fallback) and CLI plan/apply/run enrich compilation with them. `cached` is executable: verified cross-environment adoption (Iceberg snapshot identity checked at plan and run time) with build fallback on stale evidence — covered live on Nessie. Gaps: non-Iceberg sources use schema rather than data state; adoption requires the relation to be visible through the target environment's binding (true for Nessie branch inheritance); no cross-adapter reuse |
...
| 5 Nessie + WAP | partial | Nessie client boundary (REST v2, live-verified), `promote`/`rollback`/promotion artifact. `run`/`apply --ref <candidate> --from <base>` auto-creates the branch and a branch-scoped Trino catalog and writes candidate data; `plan`/`test --ref` resolve the same environment read-only. Catalog ownership is recorded so cleanup drops only catalogs Phlo created; `--environment` and `--ref` are aliases that must agree; `--nessie-catalog-uri` splits the warehouse-facing URI from the client endpoint. Promotion gates (run/tests/blocked/schema/data_diff/base/conflicts) run through one shared engine evaluation for CLI and daemon; live Nessie + Trino/Iceberg E2E covers isolation, diff, promotion, stale rejection and cleanup — including a CLI golden path (`golden_path.rs`). Gaps: no Iceberg snapshot in the promotion record, no automatic rebase of an advanced target; Nessie Iceberg catalog has no views |
...
| 8 daemon + agent APIs | partial | versioned local HTTP API, coherent snapshot, watcher/reload, `/v1/plan` with read-only environment resolution, operations API, API tests. Missing: targeted invalidation (full reload), benchmark, push diagnostics |
| dbt migration | done | `translate --from dbt` (phlo-transform-dbt): project/YAML/model/test/macro discovery, Jinja scan-classify (no execution), `ref`/`source`/`var`/`config` lowering, materialisation + incremental strategy mapping (merge/key, insert_overwrite/partition, append, watermark `is_incremental()` → `window`), generic-test conversion (`unique`/`not_null`/`accepted_values`/`relationships`/`unique_combination_of_columns` plus `dbt_utils.expression_is_true`/`accepted_range`/`not_constant`/`not_empty_string`), `CLEAN`/`REVIEW`/`UNSUPPORTED` classification, human + `--json` report, `.phlo/migration/dbt-translation.json` manifest, `--out`/`--overwrite`/`--verify` (non-zero exit when the generated workspace fails `check`). Compat pass: native CSV seeds (`seeds/**/*.csv` copy + `[seeds]`/`[seed.*]` schema + content-hash state + DuckDB `load_csv`), static `{% set %}`/`{% for %}`/`{% if %}` expansion, static lowering of `dbt_utils.generate_surrogate_key`/`star`/`safe_cast`, `dbt_date.get_base_dates`, `dbt.current_timestamp`/`date_trunc`, and restricted project-macro inlining (incl. `adapter.dispatch` → `default__` variants) — 100% of jaffle_shop_duckdb and canvas-exemplar models convert CLEAN. See `docs/dbt-migration.md` and `docs/release-validation.md`. Gaps: dynamic Jinja/macros and `run_query` are REVIEW (never executed); exposures, metrics, semantic models, snapshots, `unit_tests`/`groups` unsupported; dbt `source()`/model `meta` beyond owner lands only in the manifest |
| second adapter | done | `phlo-transform-duckdb` implements the `Adapter` trait over embedded DuckDB (bundled); `--adapter duckdb`/`--duckdb-path` selects it in the CLI; merge is emulated as delete+insert; tested in-memory, file-backed and end-to-end via a translated dbt project including incremental second runs (see `docs/validation.md`). Gaps: no Iceberg/Nessie awareness (source_state is a schema + row-count fingerprint — in-place updates are invisible), no remote cancellation |
| developer UX | partial | `explain`, `doctor`, `init`, `manifest` commands added; `--adapter` selection and clearer adapter errors. Gaps: no `fmt` command (no native formatter exists), plan output unchanged |
